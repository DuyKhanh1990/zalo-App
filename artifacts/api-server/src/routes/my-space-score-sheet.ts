...
router.get("/my-space/score-sheet", async (req, res): Promise<void> => {
  if (!requireCrm(req, res)) return;
  try {
    const data = await crmFetch(req.student.centerId, req.student.crmToken, "/api/my-space/score-sheet");
    res.json(data);
  } catch (err) {
    console.error("[crm-proxy] score-sheet failed:", (err as Error).message);
    res.status(502).json({ error: "Không thể lấy bảng điểm từ hệ thống CRM" });
  }
});
