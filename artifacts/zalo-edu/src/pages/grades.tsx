...
function hasTongCategory(scores: ScoreCategory[] | null): boolean {
  if (!scores) return false;
  return scores.some((c) => {
    const name = (c.categoryName ?? "").toLowerCase().trim();
    return name === "tổng" || name === "tổng cộng" || name === "total" || name.startsWith("tổng ");
  });
}

function avgScore(scores: ScoreCategory[] | null): number | null {
  if (!scores || scores.length === 0) return null;
  // If a "Tổng" category exists, use its value directly as the badge score
  // instead of computing an incorrect average across all categories.
  const tongCat = scores.find((c) => {
    const name = (c.categoryName ?? "").toLowerCase().trim();
    return name === "tổng" || name === "tổng cộng" || name === "total" || name.startsWith("tổng ");
  });
  if (tongCat) return parseScore(tongCat.score);
  // Otherwise compute average from all categories that have a score
...
async function fetchScoreSheet(): Promise<ScoreSheetRow[]> {
  return apiFetch<ScoreSheetRow[]>("/api/my-space/score-sheet");
}
    ...
    <div className="bg-white rounded-2xl shadow-sm overflow-hidden">
      ...
      <div className="px-4 pt-4 pb-3 flex items-start gap-3">
        <div className="flex-1 min-w-0">
          ...
          {/* Student name */}
          {(sheet.student?.name || fallbackName) && (
        ...
        {/* Avg score box — only shown when there is a real score */}
        {avg != null && avgColor && (
          <div
      ...
      {/* Expand button — only shown when there is actual score data */}
      {hasScores && (
        <button
