...
import { isInsideZMP, pickMediaFiles } from "@/lib/zmp-sdk";
import { FileViewerSheet, type FileEntry } from "@/components/file-viewer";
import { ExamSheet } from "@/components/exam-sheet";
import { apiUploadFile, getToken } from "@/lib/api-client";
...
// ─── Submission view popup ────────────────────────────────────────────────────

function SubmissionViewPopup({
  item,
  onClose,
}: {
  item: AssignmentRow;
  ...
  const attachments = normalizeSubmissionAttachments(item.submissionAttachments);
  ...
  return createPortal(
    <>
      <div className="pointer-events-auto absolute inset-0 flex flex-col justify-end" style={{ zIndex: 50 }}>
        <div
          className="absolute inset-0"
          style={{ background: "rgba(0,0,0,0.45)" }}
          onClick={onClose}
        ...
        >
          ...
          <div className="flex items-center justify-between px-5 py-3 border-b border-slate-100 flex-shrink-0">
            <div className="flex items-center gap-2">
              ...
              <span className="font-bold text-slate-800 text-base">Bài nộp của bạn</span>
            </div>
            <button
              onClick={onClose}
              className="w-8 h-8 flex items-center justify-center rounded-full hover:bg-slate-100 transition-colors"
          ...
          <div className="flex-1 overflow-y-auto px-5 py-4 space-y-4">
            ...
            {item.submissionContent && (
              ...
              >
                <div className="flex items-center gap-1.5 mb-2">
                  ...
                  <span className="text-[11px] font-bold text-indigo-600 uppercase tracking-wider">Nội dung bài làm</span>
                </div>
                <div className="text-sm text-indigo-900 leading-relaxed" dangerouslySetInnerHTML={{ __html: item.submissionContent }} />
              </div>
            )}
              ...
              <div>
                ...
                <div className="space-y-2">
                  {attachments.map((att, i) => (
                    <button
                      key={i}
                      onClick={() => att.url && setViewingFile({ url: resolveAttachmentUrl(att.url), name: att.name })}
                      className="w-full flex items-center gap-3 p-3 rounded-xl border border-indigo-100 bg-indigo-50 active:bg-indigo-100 transition-all text-left"
                    >
                      <div className="w-8 h-8 rounded-lg flex items-center justify-center flex-shrink-0" style={{ background: "#6366f120" }}>
                        {fileIconByName(att.name)}
                      </div>
                      <span className="flex-1 min-w-0 text-sm font-semibold text-indigo-700 truncate">{att.name}</span>
                      <ExternalLink size={13} className="text-indigo-400 flex-shrink-0" />
                    </button>
                  ))}
                </div>
              </div>
            ...
            {/* Empty state */}
            {!item.submissionContent && attachments.length === 0 && (
              <div className="flex flex-col items-center justify-center py-8 text-center">
                <FileText size={28} className="text-slate-300 mb-2" />
                <p className="text-sm text-slate-400">Không có nội dung bài nộp</p>
              </div>
            )}
          </div>
        </motion.div>
...
function resolveAttachmentUrl(url: string): string {
  // Return raw URL — resolveAndProxy() in FileViewerSheet handles
  // relative-path resolution (against CRM domain) and auth decisions.
  return url;
}
...
const UPLOAD_PATH = "/api/my-space/assignments/student/upload-attachment";
...
function SubmitSheet({
  ...
  const [viewingFile, setViewingFile] = useState<FileEntry | null>(null);
  // selectedFiles: File[] picked via <input> (non-ZMP path)
  const [selectedFiles, setSelectedFiles] = useState<File[]>([]);
  // zmpUploadedUrls: URLs already uploaded via openMediaPicker (ZMP path)
  const [zmpUploadedUrls, setZmpUploadedUrls] = useState<{ name: string; url: string }[]>([]);
  ...
  function removeZmpFile(index: number) {
    setZmpUploadedUrls((prev) => prev.filter((_, i) => i !== index));
  }

  async function handleTabClick(tab: "text" | "camera" | "gallery") {
    setActiveTab("text");
    setUploadError(null);

    if (tab !== "camera" && tab !== "gallery") return;

    if (isInsideZMP()) {
      // ZMP path: use native media picker with server upload
      const token = getToken();
      const uploadUrl = token
        ? `${(import.meta.env.VITE_API_BASE_URL as string | undefined)?.replace(/\/$/, "") ?? ""}${UPLOAD_PATH}?token=${encodeURIComponent(token)}`
        : undefined;

      setUploading(true);
      try {
        const pickerType = (tab === "camera" ? "zcamera_photo" : "photo") as Parameters<typeof pickMediaFiles>[0]["type"];
        const urls = await pickMediaFiles({
          type: pickerType,
          maxSelectItem: tab === "gallery" ? 5 : 1,
        ...
        if (urls && urls.length > 0) {
          ...
          setZmpUploadedUrls((prev) => [...prev, ...items]);
        }
      } catch (err) {
        setUploadError("Không thể chọn ảnh. Vui lòng thử lại.");
        console.error("[media-picker] error:", err);
      } finally {
        setUploading(false);
      }
    } else {
      // Non-ZMP: use regular file input
      if (tab === "camera") {
  ...
  async function handleSubmit() {
    ...
    // 1. Upload any File objects (non-ZMP path)
    let uploadedFromInput: string[] = [];
    if (selectedFiles.length > 0) {
      setUploading(true);
      try {
        const results = await Promise.all(
          selectedFiles.map((f) =>
            apiUploadFile<{ url: string }>(UPLOAD_PATH, f)
              .then((r) => r.url)
              .catch(() => null)
          )
        );
        uploadedFromInput = results.filter((u): u is string => !!u);
      } catch (err) {
    ...
    // 2. Combine all attachment URLs
    const allUrls = [
      ...zmpUploadedUrls.map((f) => f.url),
      ...uploadedFromInput,
    ];
    ...
    mutation.mutate(
      {
        ...
        submissionAttachments: allUrls.length > 0 ? allUrls : undefined,
        month: monthParam,
      },
      {
        onSuccess: () => {
    ...
    <div className="pointer-events-auto absolute inset-0 flex flex-col justify-end">
      ...
      >
        ...
        <div className="flex-1 overflow-y-auto">
          ...
          {/* ZMP uploaded files */}
          {zmpUploadedUrls.length > 0 && (
            <div className="px-5 pb-3">
              <p className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-2">
                Ảnh đã chọn ({zmpUploadedUrls.length})
              </p>
              <div className="flex flex-col gap-1.5">
                {zmpUploadedUrls.map((file, i) => (
                  <div key={i} className="flex items-center gap-2.5 p-2 rounded-lg border border-slate-100 bg-slate-50">
                    ...
                    <span className="flex-1 min-w-0 text-[11px] font-semibold text-slate-700 truncate">{file.name}</span>
                    <button
                      onClick={() => removeZmpFile(i)}
          ...
          {/* Upload error */}
          {uploadError && (
            <div className="mx-5 mb-3 px-3 py-2 rounded-xl bg-red-50 border border-red-100">
          ...
          <div className="px-5 pb-4 flex items-center gap-2 text-xs text-slate-400">
            <Paperclip size={13} />
            <span>Chọn ảnh bằng tab "Chụp ảnh" hoặc "Thư viện" (tối đa 10MB mỗi file)</span>
          </div>
        </div>
        ...
        <div className="px-5 py-4 border-t border-slate-100 flex-shrink-0">
          <button
            ...
            disabled={mutation.isPending || uploading || done}
...
function HomeworkCard({
  ...
  const submissionFiles = normalizeSubmissionAttachments(item.submissionAttachments);
  ...
  return (
    ...
    >
      ...
      <div className="flex items-stretch">
        ...
        <div className="flex-1 px-3.5 pt-3.5 pb-3 min-w-0">
          ...
          <div className="flex items-center gap-1.5 flex-wrap">
            ...
            {isSubmitted && (item.submissionContent || submissionFiles.length > 0) && (
              <button
              ...
              >
                <Eye size={11} />
                Bài nộp {submissionFiles.length > 0 && `· ${submissionFiles.length} file`}
              </button>
            )}
        ...
        {/* Right: score + expand toggle (only for resubmit) */}
        <div className="flex flex-col items-center justify-between flex-shrink-0">
          ...
          {/* Expand toggle — only for resubmit */}
          {canResubmit && (
            <button
      ...
      {/* Inline submit button for pending BTVN */}
      {canSubmit && (
      ...
      {/* Expand area: nộp lại (submitted BTVN only) */}
      <AnimatePresence initial={false}>
        {expanded && canResubmit && (
          <motion.div
...
export default function Homework() {
  ...
  const [activeExam, setActiveExam] = useState<{ examId: string; classId: string; title: string } | null>(null);
  ...
  return (
    <div className="flex flex-col h-full" style={{ background: "#f5f5fb" }}>
                    ...
                    {groupItems.map((item, i) => (
                      ...
                      >
                        <HomeworkCard
                          ...
                          onExam={(examId, classId, title) => setActiveExam({ examId, classId, title })}
                        />
                      </motion.div>
                    ))}
                  </AnimatePresence>
      ...
      {/* Exam sheet — in-app exam for "Bài kiểm tra" type */}
      <AnimatePresence>
        {activeExam && (
          <ExamSheet
